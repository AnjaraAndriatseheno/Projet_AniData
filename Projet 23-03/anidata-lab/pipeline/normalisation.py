...
OUTPUT_DIR = "output"
INPUT_FILE = os.path.join(DATA_CLEAN_DIR, "anime_cleaned.csv")
...
if not os.path.exists(INPUT_FILE):
    ...
    print(f"   Lancez d'abord : python cleaning.py{C.END}")
    sys.exit(1)
...
if "score" in df.columns and "members" in df.columns:
    ...
    top5 = df.nlargest(5, "weighted_score")[[name_col, "score", "members", "weighted_score"]]
...
if "score" in df.columns:
    ...
    for cat, count in dist.items():
        ...
        bar = "█" * int(pct / 2)
        print(f"    {str(cat):12s} : {count:>6,} ({pct:5.1f}%) {bar}")
else:
...
if studio_col:
    df["main_studio"] = df[studio_col].astype("string").str.split(", ").str[0]
...
if genre_col:
    df["n_genres"] = df[genre_col].astype("string").str.split(", ").str.len()
    df.loc[df[genre_col].isna(), "n_genres"] = np.nan

    df["main_genre"] = df[genre_col].astype("string").str.split(", ").str[0]
...
if duration_col:
    ...
    if df[duration_col].dtype == object or str(df[duration_col].dtype) == "string":
...
original_cols = pd.read_csv(INPUT_FILE, sep=";", nrows=0).columns
...
df.to_csv(GOLD_FILE, index=False, encoding="utf-8", sep=";")
